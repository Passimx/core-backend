...
export class CryptoUtils {
  public static importRSAKey(
    ...
    try {
      const jsonWebKey =
        typeof key === 'string' ? (JSON.parse(key) as JsonWebKey) : key;
  ...
  public static exportEncryptedPayload(
    payload: ArrayBuffer,
  ): string | undefined {
    try {
  ...
  public static getHash(value: string): string {
    return createHmac('sha256', Envs.app.appSalt).update(value).digest('hex');
  }
}
